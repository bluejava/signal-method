...
Upgrade a Signal-enabled project from its adopted methodology version to the current methodology version without overwriting project-specific truth.
...
1. Start or resume the upgrade workflow.
   ...
   - Agent actions: read `signal-method.json`, then read the active workflow state from the configured docs root.
...
3. Identify required migrations.
   ...
   - Agent actions: list each sequential migration guide that must be applied.
